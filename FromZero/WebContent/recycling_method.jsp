...
<%@ include file="./fz_header.jsp" %>
	...
	<table style="margin: 0 auto; width: 80%; background-color:white">
			<tr>
				...
				<td><div id="label-container" style="display: inline-block; width: 400px; padding: 100px;" ></div></td>
			</tr>
	</table>
	...
	<script type="text/javascript">
		...
		async function predict() {
			...
			if(prediction[0].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '플라스틱류'+'</br></br>'+'페트병과 플라스틱 용기는 안에 내용물을 깨끗이 비우고,'
				...
				labelContainer.childNodes[0].innerHTML = '비닐류'+'</br></br>'+'과자, 라면봉지, 1회용 비닐봉투에 음식물과 이물질이 묻었다면'
				+' 물로 2~3번 헹궈 잔여물을 없애고 버리고,'
				+' 이물질 제거가 어려운 경우에는 종량제봉투에 배출하시면 됩니다.';
			}
			
			else if(prediction[2].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '유리류'+'</br></br>'+'탄산음료병이나 맥주병, 소주병은 담배꽁초와 같은 이물질을 넣지 말고 버려주세요.'
				+' 하지만 거울, 깨진 유리, 도자기류,유리 식기류는 유리병류가 아니기 때문에'
				+' 종량제봉투나 전용 마대에 버려주세요.';
			}
			
			else if(prediction[3].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '캔류'+'</br></br>'+'내용물을 비우고 물로 헹구는 등 이물질을 제거하여 배출해 주세요.'
				+' 플라스틱 뚜껑 등 금속캔과 다른 재질은 제거한 후 배출합니다.';
			}
			
			else if(prediction[4].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '종이류'+'</br></br>'+'스프링 등 종이륭화 다른 재질은 제거 후 배출해 주세요.';
			}
			
			else if(prediction[5].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '종이팩'+'</br></br>'+'종이팩은 일반 종이류와 구분하여 종이팩 전용 수거함에 따로 배출해 주세요.'
				+' 수거함이 없는 경우 묶어 종이류로 배출해 주세요.';
			}
			
			else if(prediction[6].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '건전지'+'</br></br>'+'폐건전지는 녹슬지 않게 물기를 제거하고 원상태로 분리수거함에 배출해 주세요.';
			}
			
			else if(prediction[7].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '폐가전류'+'</br></br>'+'폐가전제품 무상 방문 수거 서비스를 이용하기 위해서는, 우선 폐가전제품 배출예약시스템(http://www.15990903.or.kr/ )에 접속해서 수거 예약을 해야 합니다. 냉장고, 세탁기, 에어컨, TV, 런닝머신, 전자레인지, 데스크탑PC까지 대부분의 품목을 신청할 수 있습니다.'
				+' 일반적으로 시·군·구청에 폐기물 신고를 하거나 주민센터에서 폐기물 스티커를 구입한 다음에 가전제품에 붙여서 배출해야 합니다.';
			}
			
			else if(prediction[8].probability.toFixed(2) >= 0.90) {
				labelContainer.childNodes[0].innerHTML = '일반쓰레기'+'</br></br>'+'일반 생활 쓰레기는 각 동별로 정해진 배출일의 저녁 8시 ~ 12시에 해당 지역의 청소대행업체 종량제봉투를 사용하여 배출해 주세요.';
			}
